...
namespace Movies.Client.Services
{
    public class PartialUpdateService : IIntegrationService
    {
        ...
        {
            //await PatchResource();
            PatchResourceShortcut();
        }         

        public async Task PatchResource()
        {
            //var patchDoc = new JsonPatchDocument(); // only for dynamic types if we can't use typed version
        ...
        public async Task PatchResourceShortcut()
        {
            ...
            var response = await _httpClient.PatchAsync("api/movies/5b1c2b4d-48c7-402a-80c3-cc796ad49c6b", new StringContent(JsonConvert.SerializeObject(patchDoc), Encoding.UTF8, "application/json-patch+json"));
            response.EnsureSuccessStatusCode();
